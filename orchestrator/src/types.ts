...
export interface ClaimSession{id:string;claimant_id:string;incident_group_id:string;status:SessionStatus;current_phase:Phase;claim_data:ClaimData;requires_followup:boolean;started_at:string;last_active_at:string;completed_at:string|null}
...
export type OrchestratorToClientEvent={type:"audio_out";audio:string}|{type:"transcript_partial"|"transcript_final";text:string;speaker:"user"|"agent"}|{type:"phase_update";phase:Phase}|{type:"completeness_update";completeness:import("@insuranos/schema").CompletenessResult}|{type:"photo_requested";photo_type:string}|{type:"session_paused"}|{type:"session_resumed"}|{type:"report_ready";report_url:string|null;summary_text:string}|{type:"cross_insurer_update";related_session_count:number;conflicts:string[];grounded_narrative:string}|{type:"error";message:string};
