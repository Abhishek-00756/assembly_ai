...
export function buildReport(claim:ClaimData,completeness:CompletenessResult){
 ...
 const report_json:ReportJson={
  ...
  context_factors:claim.context_factors?{"External context score":claim.context_factors.context_score==null?"Not available":String(claim.context_factors.context_score),"Weather provider":claim.context_factors.provider_status.weather,"Traffic provider":claim.context_factors.provider_status.traffic,"Weather":" "+(claim.context_factors.weather?.description??"Not available"),"Precipitation":" "+(claim.context_factors.weather?.precipitation_mm==null?"Not available":String(claim.context_factors.weather.precipitation_mm)+" mm"),"Visibility":" "+(claim.context_factors.weather?.visibility_m==null?"Not available":String(claim.context_factors.weather.visibility_m)+" m"),"Wind":" "+(claim.context_factors.weather?.wind_kmh==null?"Not available":String(claim.context_factors.weather.wind_kmh)+" km/h"),"Traffic incidents":claim.context_factors.traffic?.incident_count==null?"Not available":String(claim.context_factors.traffic.incident_count),"Risk factors":claim.context_factors.factors.join("; ") }:null,
  cross_party_context:claim.cross_party_context?{"Incident group":claim.cross_party_context.incident_group_id,"Related sessions":String(claim.cross_party_context.related_session_count),"Grounded narrative":claim.cross_party_context.grounded_narrative,"Consistency conflicts":claim.cross_party_context.conflicts.length?claim.cross_party_context.conflicts.join("; "):"None"}:null,
