...
function edgesFromSession(s:ClaimSession,sourceTool:string):GraphEdge[]{
 ...
 add("incident:"+id,"occurred_at",c.incident.date_time);add("incident:"+id,"located_at",c.incident.location);add("incident:"+id,"description",c.incident.description_summary);add("incident-group:"+s.incident_group_id,"occurred_at",c.incident.date_time);add("incident-group:"+s.incident_group_id,"located_at",c.incident.location);add("incident-group:"+s.incident_group_id,"description",c.incident.description_summary);add("incident:"+id,"weather_reported",c.incident.weather);add("incident:"+id,"road_condition_reported",c.incident.road_conditions);
...
export class GraphStore{
 async syncSession(session:ClaimSession,sourceTool:string){
  ...
  if(supabase){
   await supabase.from("knowledge_graph_edges").delete().eq("session_id",session.id).throwOnError();
   if(incoming.length){const rows=incoming.map(e=>({id:e.id,session_id:e.session_id,incident_group_id:e.incident_group_id,subject:e.subject,relation:e.relation,object_value:e.object_value,source_tool:e.source_tool,created_at:e.created_at}));await supabase.from("knowledge_graph_edges").insert(rows).throwOnError()}
  }else{
